...
import com.example.promly.SpannedGridLayout.InvalidMaxSpansException
import com.example.promly.SpannedGridLayout.InvalidSpanSizeException
import com.example.promly.SpannedGridLayout.intersects
import com.example.promly.SpannedGridLayout.isAdjacentTo
...
open class SpannedGridLayoutManager(
    ...
    /**
     * Helper get free rectangles to place views
     */
     ...
     */
    val size: Int get() = if (orientation == LinearLayoutManager.VERTICAL) height else width
    ...
    protected open fun measureChild(position: Int, view: View) {
        ...
        val usedSpan = if (orientation == LinearLayoutManager.HORIZONTAL) spanSize.height else spanSize.width
...
open class RectsHelper(
    private val layoutManager: SpannedGridLayoutManager,
    val orientation: Int
) {
    ...
    private val rectComparator = Comparator<Rect> { rect1, rect2 ->
        when (orientation) {
            ...
            else -> { Log.d("Tag","No idea what to put here")}
        }
    ...
    init {
        ...
        val initialFreeRect = if (orientation == LinearLayoutManager.VERTICAL) {
    ...
    fun pushRect(position: Int, rect: Rect) {
        val start = if (orientation == LinearLayoutManager.VERTICAL)
            rect.top else
            rect.left
        ...
        val end = if (orientation == LinearLayoutManager.VERTICAL)
            rect.bottom else
            rect.right
